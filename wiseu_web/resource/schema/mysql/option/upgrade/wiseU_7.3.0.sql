...
ALTER TABLE NVMOBILECONTENTS MODIFY KAKAO_TMPL_CD VARCHAR(50);
ALTER TABLE NVMOBILECONTENTS MODIFY CONTS_NM VARCHAR(200);

-- 카카오 탬플릿 코드 변경(브랜드톡이 50자리)
ALTER TABLE NVCAMPAIGN MODIFY KAKAO_TMPL_CD VARCHAR(50);
ALTER TABLE NVECAREMSG MODIFY KAKAO_TMPL_CD VARCHAR(50);
ALTER TABLE NVKAKAORPTSENDRESULT MODIFY TMPL_CD VARCHAR(50);


-- 브랜드톡 에러코드가 5자리
ALTER TABLE NVSENDLOG MODIFY ERROR_CD VARCHAR(5);
ALTER TABLE NVECARESENDLOG MODIFY ERROR_CD VARCHAR(5);
-- 문자/카카오톡 메세지 일부 저장
...
INSERT INTO NV_CD_MST (CD_CAT, CD, PAR_CD_CAT, VAL, CD_DESC, USE_COL, USE_YN, REG_DTM, MOD_DTM, CD_ORD, LANG)
VALUES ('AB0001', '900', 'AB00', '핸들러 오류', '핸들러 오류', NULL, 'y', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), NULL, NULL, 'ko');
INSERT INTO NV_CD_MST (CD_CAT, CD, PAR_CD_CAT, VAL, CD_DESC, USE_COL, USE_YN, REG_DTM, MOD_DTM, CD_ORD, LANG)
