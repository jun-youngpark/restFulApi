...
function setConfirmCampaign(returnValue) {
    if(returnValue == 0) {
        ...
        //sendAjaxCall("/editor/C/c_editor.do", "PUT", true , "editorFrm", null, setConfirmHandler);
        var param = $.mdf.serializeObject('#editorFrm');
        $.mdf.postJSON("/editor/updateEditorHandler.json?type=C&channelType=C", JSON.stringify(param), function(returnValue) {
...
function setConfirmHandler(returnValue) {
    if(returnValue == 0) {
        ...
        //EditorCampaignService.updateEditorCampaignKakaoTemplate(no, segmentNo, template, seg, handlerType, parent.kakaoButtons, setConfirmTemplate);
        //sendAjaxCall("/editor/C/template/c_editor.do", "PUT", true , "editorFrm", null, setConfirmTemplate);
        var param = $.mdf.serializeObject("#editorFrm");
        $.mdf.postJSON("/editor/updateEditorTemplate.json?type=C&channelType=C", JSON.stringify(param), function(returnValue) {
...
function getTemplateList(no, segmentNo) {
    //getAjaxCall("/editor/" +no + "/" + segmentNo +"/C/c_editor.do", function(templateVoList) {
    var param = {
        no : no,
        segmentNo : segmentNo
    };

    $.mdf.postJSON("/editor/selectEditorPhoneTemplate.json?type=C&channelType=C", JSON.stringify(param), function(templateVoList) {
