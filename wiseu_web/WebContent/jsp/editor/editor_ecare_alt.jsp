...
<script type="text/javascript">
        ...
        setup : function(ed) {  // tinyMCE 렌더링 완료 후 추가된 이벤트들
            ed.onKeyUp.add(function(ed, evt) {
                getLengthAndLimitLength("chkByte", tinyMCE.get("template").getContent({ format: "text"}), 2000);
            });
    ...
    function editorSave(allSave){
       ...
       var contsNo = $('#contsNo').val()
       param.templateVos= makeKkoTemplate(ecareNo, template, " ", param.kakaoSenderKey, param.kakaoTmplCd, contsNo);  //템플릿
    ...
    function getContentsList(){
         var param = $.mdf.serializeObject('#altalkContentsFrm');
...
<form id="editorFrm" name="editorFrm" method="post">
    ...
    <input type="hidden" name="contsNo" id="contsNo" value="" />
         ...
         <div class="col-9"><!-- message list -->
            <div class="table-responsive gridWrap">
                   <table class="table table-sm dataTable">
                       <tr>
                            <th scope="row"><em class="required"></em><spring:message code="ecare.menu.yellowid" /></th><!-- 플러스친구ID -->
                            <td>
                                <select id="kakaoSenderKey" name="kakaoSenderKey" class="form-control form-control-sm" onFocus='this.initialSelect = this.selectedIndex;' onChange='this.selectedIndex = this.initialSelect;'>
                                    <c:forEach var="kakaoProfileVo" items="${kakaoProfileList}" varStatus="status">
                                        <option value="${kakaoProfileVo.kakaoSenderKey}" ${kakaoProfileVo.kakaoSenderKey eq ecareScenarioVo.ecareVo.kakaoSenderKey?"selected":""}>
                                            ${kakaoProfileVo.kakaoYellowId}
                                        </option>
                                    </c:forEach>
                                </select>
                            </td>
                            <th scope="row" class="ls--1px"><em class="required"></em><spring:message code="ecare.menu.kakaotmplcd" /></th><!-- 카카오 템플릿 코드 -->
                            <td>
                                <input type="text" id="kakaoTmplCd" name="kakaoTmplCd" class="form-control form-control-sm" readonly="true" value="${ecareScenarioVo.ecareVo.kakaoTmplCd}" />
                            </td>
                       </tr>
                       <tr>
                           <td colspan="4">
                             <table id="kakao_button_table" class="table table-sm dataTable table-fixed" style="margin: -1px !important;">
                                    <colgroup>
                                        <col width="10%" />
                                        <col width="10%" />
                                        <col width="15%" /><!-- 버튼명 -->
                             ...
                             <table id="quick_reply_table" class="table table-sm dataTable table-fixed" style="margin: -1px !important;">
                                    <colgroup>
                                        <col width="10%" />
                                        <col width="10%" />
                                        <col width="15%" /><!-- 버튼명 -->
            ...
            <div class="tab-content"><!-- tab content -->
                <div class="tab-pane active" id="templateTab" role="tabpanel" aria-labelledby="template-tab"><!-- 템플릿 tab -->
                    <div id="tmplWrap"><!-- 템플릿 리스트 -->
                     </div>
                    <!-- <iframe name="smsContentsList" id="smsContentsList" src="about:blank" width="100%" frameborder="0" scrolling="no"></iframe> -->
                </div>
...
<form name="altalkContentsFrm" id="altalkContentsFrm" method="post"><!-- /editor/altalk_contents_list.do -->
    <input type="hidden" name="fileType" value="A">
    <input type="hidden" name="kakaoInspStatus" value="APR">
