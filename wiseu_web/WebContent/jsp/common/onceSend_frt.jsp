<%-------------------------------------------------------------------------------------------------
 * - [이케어>단건발송] 단건발송 메세지 작성 화면 출력 (친구톡) - MOKA에서 사용
 * - URL : /ecare/onceSend.do
 * - Controller : com.mnwise.wiseu.web.common.web.SendMappingController
 * - 이전 파일명 : send_once_frtalk.jsp
---------------------------------------------------------------------------------------------------%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="/jsp/include/taglibs.jsp" %>
<html>
<head>
<title><spring:message code="common.send.type.once"/> - <spring:message code="common.channel.C"/></title><!-- 단건발송 - 친구톡 -->
...
<script type="text/javascript">
    ...
    function initEventBind() {
        ...
        $("#sendBtn").on("click", function(event) {
            ...
            var contsNo = $("input[name='ecareVo.kakaoImageNo']").val();
            var adFlag = $("input:radio[name=adFlag]:checked").val();

            editorIfrm.sendOnce(channel,serviceNo,send_dtm,callBack,phoneNum,sendDt,sendTm,subject,kakaoSenderKey,tmplCd,smsSndYn,contsNo,adFlag);
        });
    }
    ...
    function goSubmit() {
        ...
        $("#sendOnceFrm").attr('action', '/ecare/onceSend.do?channel=C&serviceNo=8').submit();  // /ecare/send_once.do
    }
</script>
</head>
    ...
    <div class="container-fluid mt-4 mb-2">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0"><spring:message code="common.send.type.once"/> - <spring:message code="common.channel.C"/></h3><!-- 단건발송 - 친구톡 -->
            </div>
            ...
            <input type="hidden" name="serviceNo" value="8"/>
            <input type="hidden" name="ecareVo.templateSenderKey" id="ecareVo.templateSenderKey" />
            <input type="hidden" name="ecareVo.kakaoImageNo" id="ecareVo.kakaoImageNo" />
            ...
            <div class="card-body">
                <ul class="nav nav-tabs tab02 mb-4" id="tabs-text" role="tablist"><!-- tabs -->
                    <c:forEach var="item" items="${wiseu:getProperty('channel.use.list', '')}">
                        ...
                        <c:if test="${item eq 'C'}">
                            <li class="nav-item"><!-- /ecare/send_once.do -->
                                <a class="nav-link mb-sm-3 mb-md-0 active" href="/ecare/onceSend.do?serviceNo=8&channel=C"><spring:message code="common.channel.C"/></a>
                            </li>
                        </c:if>
                    </c:forEach>
                </ul>
                ...
                <div class="table-responsive gridWrap">
                    <table class="table table-sm dataTable table-fixed">
                        ...
                        <tr>
                            ...
                            <th scope="row"><em class="required">required</em><spring:message code="campaign.menu.prepend.ad.yn"/></th><!-- 광고문구표기여부 -->
                            <td>
                                <div class="form-inline">
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" value="Y" name="adFlag" id="adFlag_Y" class="custom-control-input" />
                                        <label class="custom-control-label" for="adFlag_Y"><spring:message code="campaign.menu.item.prepend.ad.y"/></label><!-- 표기함 -->
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" value="N" name="adFlag" id="adFlag_N" class="custom-control-input" />
                                        <label class="custom-control-label" for="adFlag_N"><spring:message code="campaign.menu.item.prepend.ad.n"/></label><!-- 표기 안함 -->
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="control-text-sm">
                                <div class="form-inline">
                                    <input type="text" class="form-control form-control-sm" name="phoneNum" id="phoneNum" />
                            ...
                            <td>
                                <div class="form-inline">
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" value="Y" name="smsSndYn" id="smsSndYn_Y" class="custom-control-input" />
                                        <label class="custom-control-label" for="smsSndYn_Y"><spring:message code="ecare.menu.item.sms.send.y"/></label><!-- 발송함 -->
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" value="N" name="smsSndYn" id="smsSndYn_N" class="custom-control-input" />
            ...
            <div class="card-body message_contents"><!-- 에디터 -->
                <iframe id="editorIfrm" src="/editor/editor.do?type=ecare&ecareNo=0&serviceType=C&handlerType=G&channelType=T&subType=S&sendOne=O" scrolling="no" name="editorIfrm" frameborder="0" width="100%"></iframe>
            </div>
        </div>
