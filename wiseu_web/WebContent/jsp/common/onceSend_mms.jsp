<%-------------------------------------------------------------------------------------------------
 * - [이케어>단건발송] 단건발송 메세지 작성 화면 출력 (LMS) - MOKA에서 사용
 * - URL : /ecare/onceSend.do
 * - Controller : com.mnwise.wiseu.web.common.web.SendMappingController
 * - 이전 파일명 : send_once_lms.jsp
---------------------------------------------------------------------------------------------------%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="/jsp/include/taglibs.jsp" %>
<html>
<head>
<title><spring:message code="common.send.type.once"/> - <spring:message code="common.channel.T"/></title><!-- 단건발송 - LMS/MMS -->
...
<script type="text/javascript">
    ...
    function goSubmit() {
        ...
        $("#sendOnceFrm").attr('action', '/ecare/onceSend.do?channel=T&serviceNo=6').submit();  // /ecare/send_once.do
    }
</script>
</head>
<body>
    ...
    <div class="container-fluid mt-4 mb-2">
        <div class="card">
            <div class="card-header">
                <h3 class="mb-0"><spring:message code="common.send.type.once"/> - <spring:message code="common.channel.T"/></h3><!-- 단건발송 - LMS/MMS -->
            </div>
            ...
            <input type="hidden" name="serviceNo" value="6"/>
            ...
            <div class="card-body">
                <ul class="nav nav-tabs tab02 mb-4" id="tabs-text" role="tablist"><!-- tabs -->
                    <c:forEach var="item" items="${wiseu:getProperty('channel.use.list', '')}">
                        ...
                        <c:if test="${item eq 'T'}">
                            <li class="nav-item"><!-- /ecare/send_once.do -->
                                <a class="nav-link mb-sm-3 mb-md-0 active" href="/ecare/onceSend.do?serviceNo=6&channel=T"><spring:message code="common.channel.T"/></a>
                            </li>
                        </c:if>
                        <c:if test="${item eq 'A'}">
                            <li class="nav-item"><!-- /ecare/send_once.do -->
                ...
                <div class="table-responsive gridWrap">
                    <table class="table table-sm dataTable table-fixed">
                        ...
                        <tr>
                            ...
                            <td colspan="3"><input type="text" class="form-control form-control-sm" name="subject" id="subject" maxlength="40" /></td>
                        </tr>
                        <tr>
           ...
           <div class="card-body message_contents"><!-- 에디터 -->
               <iframe id="editorIfrm" src="/editor/editor.do?type=ecare&ecareNo=0&serviceType=S&handlerType=G&channelType=T&subType=S&sendOne=O" scrolling="no" name="editorIfrm" frameborder="0" width="100%"></iframe>
           </div>
        </div>
