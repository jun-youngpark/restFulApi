...
<script type="text/javascript">
    ...
    function initEventBind() {
        ...
        $("#pauseBtn").on("click", function(event) {
            if($("#serviceStatus").val() == 'E') {
                alert("<spring:message code='watch.alert.re8'/>");  // 발송이 종료된 이케어는 일시정지를 할  수 없습니다.
                return;
            }
        ...
        $("#stopBtn").on("click", function(event) {
            if($("#serviceStatus").val() == 'E') {
                alert("<spring:message code='watch.alert.re9'/>");  // 발송이 종료된 이케어는 정지를 할  수 없습니다.
                return;
            }
        ...
        $("#restartBtn").on("click", function(event) {
            if($("#serviceStatus").val() == 'E') {
                alert("<spring:message code='watch.alert.re7'/>");  // 발송이 종료된 이케어는 재가동을 할  수 없습니다.
                return;
            }
    ...
    function listService() {
        ...
        $.post("/watch/serviceList.do?menu=ecare&tid=" + tid + "&serviceType=" + serviceType + "&selectNum=" + selectNumCd, null, function(result) {  // /watch/watch_list.do?cmd=listService
