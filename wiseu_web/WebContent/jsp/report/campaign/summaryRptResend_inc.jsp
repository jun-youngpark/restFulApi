<%-------------------------------------------------------------------------------------------------
 * - 이전 파일명 : campaign_report_resend_inc.jsp
-------------------------------------------------------------------------------------------------%>
...
<script type="text/javascript">
    ...
    function initResendEventBind() {
        ...
        $("#resendBtn").on("click", function(event) {
            ...
            if($.mdf.validForm("#campaignReportFrm", rules) == false) {
                return;
            }
            ...
            if(confirm(resendTargetCnt + " <spring:message code='resend.send.confirm' />") == true) {  // 건을 재발송 하시겠습니까?
                var param = $.mdf.serializeObject("#campaignReportFrm");
    ...
    function viewResendForm(resultSeq, superSeq, serviceNo, channel, failCnt) {
        ...
        var params = "resultSeq="+resultSeq+"&serviceNo="+serviceNo+"&channel="+channel+"&client=EM";
...
<input type="hidden" name="serviceNo" value="${campaignReportBasicVo.campaignNo}" />
<input type="hidden" name="channel" value="${campaignReportBasicVo.channelType}" />
<input type="hidden" name="subType" id="subType" value="S" />
