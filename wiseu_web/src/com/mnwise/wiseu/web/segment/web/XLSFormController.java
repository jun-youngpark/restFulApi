...
/**
 * XLS 대상자 파일 올리기 2단계 Controller
 */
@Controller
public class XLSFormController extends BaseController {
    private static final Logger log = LoggerFactory.getLogger(XLSFormController.class);
    ...
    public UploadSegVo formBackingObject(String importedFilePath, @RequestParam(defaultValue="") String inMetaData, @RequestParam(defaultValue="") String delimiter,
        ...
        try {
            ...
            POIFSFileSystem poif = new POIFSFileSystem(in);
            workbook = new HSSFWorkbook(poif);
            HSSFSheet sheet = workbook.getSheetAt(0);

            int totalLine = 0;
            if(sheet != null) {
                totalLine = sheet.getLastRowNum() + 1;  // getLastRowNum() 메소드는 실제 rowcount -1 이 리턴
                    ...
                    if(whileCnt == 2 && i == 0) {
                        while(cells.hasNext()) {
                            ...
                            if(cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC)
                                // 대상자 파일이라 더블형 데이터는 없다고 생각하고 처리한다 int로 변환안하면 1
                                // 이 1.0 으로 나온다..
    ...
    /**
     * [대상자 관리>대상자파일 올리기>2단계] 대상자 기본정보 및 컬럼지정 화면 출력 (XLS)
     * [공통팝업>대상자관리>2단계] 대상자 기본정보 및 컬럼지정 (팝업) 화면 출력 (XLS)
     *
     * @param request
     * @return
     * @throws Exception
     */
    // /segment/target/excelimport.do, /segment/upload/excelimport.do
    @RequestMapping(value={"/target/fileSegment2Step_xls.do", "/segment/fileSegment2Step_xls.do"}, method=RequestMethod.GET)
    public ModelAndView referenceData(HttpServletRequest request) throws Exception {
        try {
            ...
            if(request.getRequestURI().indexOf("target") > -1) {
                mav.addObject("action", "/target/fileSegment2Step_xls.do");  // /segment/target/excelimport.do
            } else {
                mav.addObject("action", "/segment/fileSegment2Step_xls.do");  // /segment/upload/excelimport.do
            }
            return mav;
        } catch (Exception e) {
            log.error(null, e);
    ...
    /**
     * [대상자 관리>대상자파일 올리기>2단계] 대상자 기본정보 및 컬럼지정 - 다음단계 버튼 클릭 (XLS)
     * [공통팝업>대상자관리>2단계] 대상자 기본정보 및 컬럼지정 (팝업) - 다음단계 버튼 클릭 (XLS)
     *
     * @param uploadSegVo
     * @param reject
     * @param request
     ...
     */
    // /segment/target/excelimport.do, /segment/upload/excelimport.do
    @RequestMapping(value={"/target/fileSegment2Step_xls.do", "/segment/fileSegment2Step_xls.do"}, method=RequestMethod.POST)
    public ModelAndView onSubmit(UploadSegVo uploadSegVo, @RequestParam(defaultValue="") String reject, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            while(rows.hasNext()) {
                ...
                if(cvsHeaders.length == cc) {
                    ...
                    if(isErr) {
                        for(int i = 0; i < cc; i++)
                            csvData[i] = ((HSSFCell) row.getCell(i)).getRichStringCellValue().getString();
        ...
        return new ModelAndView(new RedirectView((String) returnData.get("submitPage")), returnData);
    }
}
